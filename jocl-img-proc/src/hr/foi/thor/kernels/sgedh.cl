...
void hyst(__global uchar *data,__global uchar *out, const int2 imageSize)
{
	float lowThresh = 90;
...
__kernel void sgedh(__global uchar4* inputImage, __global uchar4* outputImage, const int2 imageSize){
	sobel(inputImage, outputImage);
	gauss(inputImage, outputImage);
	erode(inputImage, outputImage);
	dilate(inputImage, outputImage);
	hyst(inputImage, outputImage, imageSize);
